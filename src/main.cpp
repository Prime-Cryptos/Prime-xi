...
// Copyright (c) 20014-2017 Prime-Xi developers
...
uint256 hashGenesisBlock("0x0000015517a5f7af67f5bdcfd048f038a69161095451cce5eb8541d91bc2cf5e");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // primexi: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "primexi Signed Message:\n";
...
{
    // primexi: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 25; // Primexi: every block
static const int64 nTargetSpacing = 1 * 25; // Primexi: 25 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // primexi: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // primexi: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // primexi: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Prime X11";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1405469795;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2057848;
        ...
        assert(block.hashMerkleRoot == uint256("0xec13e92232915e5240e5cf4c34cdc4050498b292f680f0c0c334480a705ca518"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa3, 0xd5, 0xc2, 0xf9 }; // primexi: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("primexiMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("primexiMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static primexiMiner(CWallet *pwallet)
{
    printf("primexiMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("primexi-miner");
    ...
    try { while (true) {
        ...
        printf("Running primexiMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("primexiMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&primexiMiner, pwallet));
}
